...
const Card = ({ entry, date, key }: CardType) => {
  return (
    ...
    >
      <div className="leading-[19px] antialiased text-white max-h-[210px] px-3 py-4 text-ellipsis overflow-hidden font-medium">
        <p dangerouslySetInnerHTML={{ __html: entry }}></p>
      </div>
      <div className="h-[38px] px-3 py-2 border-t-[1px] border-gray-400 text-sm text-slate-200 font-medium">
        {date}
      </div>
    </div>
  );
